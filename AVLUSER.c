...
struct AVLNode
{
    ...
    int height;
    int rotationCount; // New field to track rotation
};
...
struct AVLNode* rightRotate(struct AVLNode* y)
{
    ...
    y->rotationCount++; // Increment rotation count
    return x;
}
...
struct AVLNode* leftRotate(struct AVLNode* x)
{
    ...
    x->height = max(height(x->left), height(x->right)) + 1;
    y->height = max(height(y->left), height(y->right)) + 1;
    x->rotationCount++; // Increment rotation count
    return y;
}
...
int countRotations(struct AVLNode* root)
{
    if (root == NULL)
        return 0;

    int leftRotations = countRotations(root->left);
    int rightRotations = countRotations(root->right);

    return leftRotations + rightRotations + root->rotationCount;
}
...
int main()
{
    ...
    do
    {
        ...
        printf("6. Display the rotation count\n");
        printf("7. exit\n");
